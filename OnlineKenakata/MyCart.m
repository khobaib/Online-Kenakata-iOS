...
- (void)viewDidLoad
{
    ...
    [self initLoading];
    productList=[DatabaseHandeler getProduct];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(productList.count>0){
        ...
        if(serverData){
            NSMutableDictionary *data=[serverProductList objectAtIndex:indexPath.row];
            ...
            if([spclQusText  isEqualToString:@""]){
                ...
                if(GAQ>5){
                    ...
                    [stocIndicator setTextColor:[UIColor yellowColor]];
                }else{
...
-(IBAction)quantity:(id)sender{
    ...
    NSString *spclQusText =[[serverProductList objectAtIndex:quantity.row] objectForKey:@"special_question" ];
    
    if([spclQusText  isEqualToString:@""]){
        editQuantity=[[[serverProductList objectAtIndex:quantity.row]objectForKey:@"general_available_quantity"]intValue];

    }else{
      NSArray *arr=[[serverProductList objectAtIndex:quantity.row] objectForKey:@"special_answers"];
...
- (void)pickerViewController:(RMPickerViewController *)vc didSelectRows:(NSArray *)selectedRows{
    ...
    int x=[[[serverProductList objectAtIndex:quantity.row] objectForKey:@"price"] intValue]*qNew;
